...
def build_oof_gamma_teacher_data(d_hat_train, p_train, p_bs):
    """
    train 500 내부에서 OOF 방식으로 gamma teacher 데이터를 만든다.
    각 샘플은 자기 자신을 학습하지 않은 RF/profile로부터 상태 z를 얻는다.

    중요 수정:
    1) teacher 학습 시에도 batch RF uncertainty를 계산해 최종 추론과 피처 분포를 맞춘다.
    2) inner_tune 고정 100개 대신 OOF 전체 train 500개를 사용해 leakage를 줄인다.
    """
    ...
    print()
    print("Counterfactual Gamma Teacher 데이터 생성 - train 500 내부 OOF")
...
def build_inner_gamma_teacher_data(d_hat_train, p_train, p_bs, inner_train_size=400):
    """
    train 500을 inner_train 400 / inner_tune 100으로 나누어
...
def train_four_way_expert_router(Z, gamma_target, teacher_diag, gate):
    """
    train 500 내부 OOF 결과로 RF/ML/Rule/Teacher 중 최적 후보 라벨을 만든다.
...
def resolve_mat_path():
    """
    채점 환경에서는 DH_FR1.mat가 main.py와 같은 폴더에 배치된다.
    로컬 테스트 편의를 위해 InF_DH_FR1.mat도 fallback으로 허용한다.
    """
    candidates = [MAT_PATH, "InF_DH_FR1.mat"]
    for path in candidates:
        if Path(path).exists():
            return path
    raise FileNotFoundError("DH_FR1.mat 파일을 찾을 수 없습니다. main.py와 같은 폴더에 배치하세요.")


def load_model_package(model_path=MODEL_PATH):
    if not Path(model_path).exists():
        raise FileNotFoundError("model.pkl 파일을 찾을 수 없습니다. 먼저 python train.py를 실행하세요.")
...
def apply_saved_settings(model_package):
    """
    train.py와 main.py의 주요 상수가 달라지는 실수를 막기 위해
    model.pkl에 저장된 설정값이 있으면 추론 시 전역 상수에 반영한다.
    """
    settings = model_package.get("settings", {})
    mapping = {
        "use_bin_bias_correction": "USE_BIN_BIAS_CORRECTION",
...
def your_algorithm(d_single, p_bs):
    """
    단일 사용자에 대한 최종 위치 추정 함수.
    hidden test의 정답 p는 사용하지 않고, d_hat[:, u]와 저장된 model.pkl만 사용한다.
    """
    import contextlib
    import io
    ...
    if _MODEL_PACKAGE is None:
        _MODEL_PACKAGE = load_model_package(MODEL_PATH)
        apply_saved_settings(_MODEL_PACKAGE)

    model_package = _MODEL_PACKAGE

    d_single = np.asarray(d_single, dtype=float).reshape(-1)
    d_eval = d_single.reshape(-1, 1)

    profile = model_package["profile"]
    rf_model = model_package["rf_model"]
    gamma_teacher = model_package.get("gamma_teacher", None)
    expert_router = model_package.get("expert_router", None)

    X_eval = make_profile_features(d_eval, profile)

    # predict_with_algorithm 내부의 print 출력이 채점 로그에 섞이지 않도록 숨긴다.
    with contextlib.redirect_stdout(io.StringIO()):
        _, _, _, _, _, expert_pred, _ = predict_with_algorithm(
            d_hat_eval=d_eval,
            X_eval=X_eval,
            model=rf_model,
            profile=profile,
    ...
    return np.asarray(expert_pred[0], dtype=float).reshape(2)
...
def main():
    ...
    # 3) 사용자 수는 입력에서 동적으로 결정
    num_user = d_hat.shape[1]
    p_hat = np.zeros((2, num_user), dtype=float)

    # 4) hidden test 추론. p는 로드되더라도 절대 사용하지 않는다.
    for u in range(num_user):
        p_hat[:, u] = your_algorithm(d_hat[:, u], p_bs)

    # 5) 채점기 반환 형식: numpy array, shape = (2, num_user)
    return p_hat
